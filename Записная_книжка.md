Мы хотели раскидать инфу с сайта на 4 файла, но это оказалось слишком сложно. Перекинули всё по карточке в один файл
остальные три удаляю
____________
Чтоб не потерять код карточки под шаблон для всех
    ...
    <style>
        * {
        ...
        .card-img-wrapper img {
            ...
            background: #d4edda; /* если фото не загрузится — зелёный фон */
        }
...
<body>
    <header>
        <h1>📖 Альбом событий</h1>
        <div class="filters">
            <button class="filter-btn active" data-type="all">Все</button>
            <button class="filter-btn" data-type="на время">На время</button>
            <button class="filter-btn" data-type="на материалы">На материалы</button>
            <button class="filter-btn" data-type="цветные">Цветные</button>
        </div>
    </header>
    ...
    <script>
        const events = [
            {
                ...
                "скриншоты": [
                    "Лига Авантюр 6667.png"   // <--- твой рисунок
                ]
            }
        ];
        ...
        document.addEventListener('DOMContentLoaded', function() {
            ...
            function openModal(event) {
                ...
                let galleryHtml = event.скриншоты.map(src => `<img src="${src}" alt="скрин" onclick="window.open('${src}', '_blank')" onerror="this.style.display='none'">`).join('');
